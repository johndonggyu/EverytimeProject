...
import django
django.setup()
from backend.parsed_data.models import board, board_keyword
import itertools
...
def get_tokens(match_, _date):
	...
	everytime_data = board.objects.filter(date__range=(start, end))
...
def count_word(count, _date):
	...
	for tags, counts, in count.most_common(50):
		if(len(str(tags)) > 1):
			...
			start = datetime(int(_date[0:4]),int(_date[5:7]),1)
			board_keyword(code=bc.saenaegi, keyword=tags, word_date=start, count=counts).save()
	#except:
...
if __name__ == '__main__':
	...
	if(board.objects.count() == 0):
	...
	elif(board_keyword.objects.count() == 0):
		print("키워드 테이블이 비었습니다. 바로 wordcloud 만들겠습니다.")
		draw_wordcloud(one_list(bc.saenaegi, _date), 'saenaegi'+_date, _date)
	else:
		...
		if(str(tmp.word_date)[0:7] == _date[0:7]):
			...
			if(temp == 1 or temp == "1"):
				...
				del_obj = board_keyword.objects.filter(word_date__range=(start, end),code=bc.saenaegi)
				del_obj.filter(code=bc.saenaegi).delete()
