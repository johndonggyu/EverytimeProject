...
def get_tokens_for_charts(each_Professor): #추후 구현 type_ =3, match_ = 한종배 교수님
    ...
    print (everytime_data[0].professor.professor.professor+"교수님 평균 별점 : %.2f" %(averageScore))
    print()
    ...
    for datum in everytime_data: #객체화 시킨 데이터들을 한줄씩 불러 읽음
        ...
        if strTeam_project == '많음':
            ...
            team_projectNone+=1
    print(everytime_data[0].professor.professor.professor+"교수님 팀플 분량")
    ...
    for datum in everytime_data: #객체화 시킨 데이터들을 한줄씩 불러 읽음
        ...
        if strCredit == '학점느님':
            ...
            creditFbomb+=1
    print(everytime_data[0].professor.professor.professor+"교수님 학점 비율")
    ...
    for datum in everytime_data: #객체화 시킨 데이터들을 한줄씩 불러 읽음
       ...
       if strAttendance == '혼용':
            ...
            attendanceNone+=1
    print(everytime_data[0].professor.professor.professor+"교수님 출결 어떻게 부르나요")
    ...
    for datum in everytime_data: #객체화 시킨 데이터들을 한줄씩 불러 읽음
        ...
        if strTest == '네번이상':
            ...
            testNone+=1
    print(everytime_data[0].professor.professor.professor+"교수님 강의별 시험 횟수")
...
def draw_piechart_assignment_and_teamProject(each_Professor):
    ...
    if type(alist) == type(int):
        if alist <= 0:
            ...
            return 0
    labels = ['많음', '보통','없음']
    titles = [each_Professor[0].professor_professor_professor+' 과제 분량', each_Professor[0].professor_professor_professor+' 팀플 분량']
...
def draw_barPlot_professor_TeamProject(each_Professor):
    alist = get_tokens_for_charts(each_Professor)
    if type(alist) == type(int):
        if alist <= 0:
    ...
    plt.title(each_Professor[0].professor.professor.professor+' 팀플 비율') #차트에 제목 붙이기
    outputfile_name = dir_static+each_Professor[0].professor.professor.professor+" TeamProject.png"
    plt.savefig(outputfile_name)
     #차트 이미지로 저장하기
    #plt.show()

def draw_barPlot_professor_Credit(each_Professor):
    alist = get_tokens_for_charts(each_Professor)
    if type(alist) == type(int):
        if alist <= 0:
    ...
    plt.title(each_Professor[0].professor.professor.professor+' 학점 비율') #차트에 제목 붙이기
    outputfile_name = dir_static+each_Professor[0].professor.professor.professor+" Credit.png"
    plt.savefig(outputfile_name)
    #plt.show()

def draw_barPlot_professor_Attendence(each_Professor):
    alist = get_tokens_for_charts(each_Professor)
    if type(alist) == type(int):
        if alist <= 0:
    ...
    plt.title(each_Professor[0].professor.professor.professor+' 출결 비율') #차트에 제목 붙이기
    outputfile_name = dir_static+each_Professor[0].professor.professor.professor+" Attendence.png"
    plt.savefig(outputfile_name)
    #plt.show()

def draw_barPlot_professor_Test(each_Professor):
    alist = get_tokens_for_charts(each_Professor)
    if type(alist) == type(int):
        if alist <= 0:
    ...
    plt.title(each_Professor[0].professor.professor.professor+' 시험 비율') #차트에 제목 붙이기
    outputfile_name = dir_static+each_Professor[0].professor.professor.professor+" Test.png"
    plt.savefig(outputfile_name)
    #plt.show()
...
def gauge(each_Professor, labels=['LOW','MEDIUM','HIGH','VERY HIGH','EXTREME'], \
    ...
    alist= get_tokens_for_charts(each_Professor) #db랑 연동시켜
...
for i in range(0, len(objective_list)):
    ...
    draw_barPlot_professor_TeamProject(objective_list[i])
    draw_barPlot_professor_Credit(objective_list[i])
    draw_barPlot_professor_Attendence(objective_list[i])
    draw_barPlot_professor_Test(objective_list[i])
    gauge(objective_list[i], labels=['0-1 star','1-2 stars','2-3 stars','3-4 stars','4-5 stars'], colors='cool_r', arrow=3, title=objective_list[i][0].professor.professor.professor+' 교수님 만족도', fname = './frontend/static/chart/'+objective_list[i][0].professor.professor.professor+'gaugeChart_professor_averageRate.png')
